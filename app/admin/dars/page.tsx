  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-foreground dark:text-white font-serif">إدارة الدروس العلمية</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-card p-6 rounded-2xl border border-border flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-foreground dark:text-white mt-1">
              {lessons.reduce((acc, l) => acc + (l.views_count || 0), 0).toLocaleString()}
            </p>
          </div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-border">
                {filteredLessons.map((lesson) => (
                  <tr key={lesson.id} className="hover:bg-muted/50 dark:hover:bg-background-alt/50 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold ${lesson.publish_status === "published" ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"}`}
                      >
                        {lesson.publish_status === "published" ? "منشور" : "مسودة"}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-sm text-text-muted">{lesson.views_count || 0}</td>
                    <td className="px-6 py-5">
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-3xl bg-card max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleEditLesson} className="space-y-6 mt-4">
            <div className="space-y-2">
              <Label>عنوان الدرس</Label>
              <Input
                value={formData.title}
